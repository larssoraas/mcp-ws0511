...
async def startup_event():
    ...
    logger.info("Starting MCP API Server Lab02...")

@app.on_event("shutdown")
async def shutdown_event():
    """Cleanup ved nedstengning."""
    await http_client.aclose()
    logger.info("MCP API Server Lab02 avsluttet")
...
async def get_weather(request: WeatherRequest):
    """
    Hent værprognose for en destinasjon.
    Returns MCP-compliant tool result format.
    """
    try:
        ...
        if "error" in result:
            # MCP tool execution error format
            return {
                "content": [
                    {
                        "type": "text",
        ...
        # MCP successful result format with structured content
        return {
            "content": [
                {
                    "type": "text",
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Feil ved henting av værdata: {str(e)}"
                }
            ],
            "isError": True
        }
...
if __name__ == "__main__":
    logger.info("Starting MCP Server API Lab02 on port 8000...")
